...
func FromSlog(l *slog.Logger) log.Logger {
...
func (a *slogAdapter) With(kvs ...interface{}) log.Logger {
...
func (h *engineHandler) Enabled(_ context.Context, level slog.Level) bool {
	...
	case level < slog.LevelInfo:
		return log.Enabled(log.LevelDebug)
	case level < slog.LevelWarn:
		return log.Enabled(log.LevelInfo)
	case level < slog.LevelError:
		return log.Enabled(log.LevelWarn)
	default:
		return log.Enabled(log.LevelError)
	}
}
