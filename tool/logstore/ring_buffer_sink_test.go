...
func TestRingBuffer_OverflowKeepsLatest(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_ = rb.Write(LogEntry{Time: time.Now(), Level: log.LevelInfo, Msg: "m", Fields: map[string]interface{}{"i": i}})
	}
	if rb.Len() != 3 {
...
func TestRingBuffer_QueryFilters(t *testing.T) {
	...
	now := time.Now()
	rb.Write(LogEntry{Time: now, Level: log.LevelInfo, Msg: "login", TraceID: "t1", Actor: "/user/auth"})
	rb.Write(LogEntry{Time: now.Add(time.Second), Level: log.LevelError, Msg: "login fail", TraceID: "t2", Actor: "/user/auth"})
	rb.Write(LogEntry{Time: now.Add(2 * time.Second), Level: log.LevelWarn, Msg: "deg", TraceID: "t1", Actor: "/system"})
	...
	// 最低级别 Error
	got = rb.Query(QueryFilter{MinLevel: log.LevelError})
