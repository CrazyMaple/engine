...
func TestAESGCMCipher_WrongKeyFails(t *testing.T) {
	...
	c2, _ := NewAESGCMCipher(randomKey(t), 1)
...
func BenchmarkAESGCMEncrypt_64B(b *testing.B) { benchEncrypt(b, 64) }
func BenchmarkAESGCMEncrypt_1KB(b *testing.B) { benchEncrypt(b, 1024) }
func BenchmarkAESGCMEncrypt_16KB(b *testing.B) {
	benchEncrypt(b, 16*1024)
}
func BenchmarkAESGCMDecrypt_1KB(b *testing.B) { benchDecrypt(b, 1024) }
